...
class CrossModule(nn.Module):
    def __init__(self, feature_hidden_dim):
        super(CrossModule, self).__init__()
        self.cross_attention1 = nn.MultiheadAttention(feature_hidden_dim, feature_hidden_dim)
        self.cross_specific_1 = nn.Sequential(
            nn.BatchNorm1d(feature_hidden_dim),
            nn.ReLU(),
            nn.Dropout(p=0.1)
        )
        # self.cross_attention2 = nn.MultiheadAttention(feature_hidden_dim, feature_hidden_dim)
        # self.cross_specific_2 = nn.Sequential(
        #     nn.BatchNorm1d(feature_hidden_dim*2),
        #     nn.ReLU(),
        #     nn.Dropout(p=0.1)
        # )
        self.cross_specific = nn.Sequential(
    ...
    def forward(self, text, image):
        attn_output1, _ = self.cross_attention1(text, image, image)
        correlation_out1 = self.cross_specific(attn_output1)
        # attn_output2, _ = self.cross_attention2(image, text, text)
        # correlation_out2 = self.cross_specific_2(image)
        # correlation_out = self.cross_specific(torch.cat((correlation_out1, correlation_out2), 1))
        return correlation_out1