...
from mcd_time.src.mlp import MLP
from mcd_time.src.moe_all import MoE
from mcd_time.module.base_module import CrossModule
...
class MCDModel(nn.Module):
    def __init__(self, dataTime, config, is_cold_start=False):
        super().__init__()
        self.config = config
    ...
    def filter_comments_by_time(self, comment_feas, comment_times):
        # 创建时间掩码
        time_mask = (comment_times >= 0) & (comment_times <= self.dataTime)
        # 应用掩码：将不符合时间的评论特征置为0
        filtered_comments = comment_feas * time_mask.unsqueeze(-1).float()
    ...
    def forward(self, **kwargs):
        ...
        # commenter-features
        # 获取原始评论数据
        raw_comments = kwargs["comment_feas"]  # 列表，每个元素形状不同
        raw_times = kwargs["comment_times"]    # 对应的时间
        # 在模型中过滤
        filtered_comments = self.filter_comments_by_time(raw_comments, raw_times)
        comments_fea = self.linear_comment(filtered_comments)
        ...
        if self.is_cold_start:  # 冷启动 没有评论数据
            ...
            fea = torch.concat((p_v_fea2, topics_fea), -1)  # → [256, 1, 256]
...
class MCDModel_wo_guideAttn(nn.Module):
    def __init__(self, dataNumber, config, is_cold_start=False):
        ...
        self.linear_vp = nn.Sequential(
            nn.Linear(cls_dim * 4, cls_dim * 2),
            nn.LayerNorm(cls_dim * 2),
            nn.ReLU(),
            nn.Dropout(p=dropout),
        ...
        self.reduce_dim1 = nn.Linear(cls_dim * 3, cls_dim * 3)
    ...
    def forward(self, **kwargs):
        ...
        fea = torch.concat((v_c, v_p, topics_fea), -1)  # → [B, 1, 128 * 3]
...
class MCDModel_wo_taskMOE(nn.Module):
    def __init__(self, dataNumber, config, is_cold_start=False):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(cls_dim * 7, int(cls_dim / 2)),
            nn.LayerNorm(int(cls_dim / 2)),
            nn.ReLU(),
            nn.Dropout(p=dropout),
            nn.Linear(int(cls_dim / 2), 2),
        ...
        # 回归任务头
        self.gru = GRUModel(
            input_size=cls_dim * 7,
            hidden_size=cls_dim * 7,
            num_layers=3,
            output_size=cls_dim * 7
        )
        self.regressor = nn.Sequential(
            nn.Linear(cls_dim * 7, int(cls_dim / 2)),
            nn.LayerNorm(int(cls_dim / 2)),
        ...
        self.reduce_dim1 = nn.Linear(cls_dim * 7, cls_dim)  # cls_dim * 4, cls_dim * 2 512, 256
        self.reduce_dim2 = nn.Linear(cls_dim * 2, cls_dim * 2)  # cls_dim * 4, cls_dim * 2 512, 256
        self.proj1 = nn.Linear(feature_hidden_dim * 6, feature_hidden_dim * 2)
        self.proj2 = nn.Linear(feature_hidden_dim * 2, feature_hidden_dim * 2)
