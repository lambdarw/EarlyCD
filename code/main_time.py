...
from mcd_time.utils.metrics import metrics, reg_metrics
from mcd_time.module.mcd_model import MCDModel, MCDModel_wo_taskMOE, MCDModel_wo_guideAttn
from mcd_time.utils.metrics import huber_loss
...
if __name__ == "__main__":
    dataTime = 2  # 1h, 4h, 8h, 12h, 24h, 48h
    if dataTime == 0:
        is_cold_start = True  # 是否冷启动 默认F表示不是冷启动
    else:
        is_cold_start = False
    dataset_name = 'ours'  # mmcd, ours
    task = 'MCDModel'  # MCDModel, MCDModel_wo_guideAttn, MCDModel_wo_taskMOE,

    if dataset_name == 'mmcd':
        config_path = "mcd_time/config/train.yaml"
        module = importlib.import_module("mcd_time.src.dataset")
    else:
        config_path = "mcd_time/config/ours_train.yaml"
        module = importlib.import_module("mcd_time.src.ours_dataset")
    ...
    if task == 'MCDModel':
        model = MCDModel(dataTime, config.model, is_cold_start)
    elif task == 'MCDModel_wo_taskMOE':
        model = MCDModel_wo_taskMOE(dataTime, config.model, is_cold_start)
    else:
        model = MCDModel_wo_guideAttn(dataTime, config.model, is_cold_start)
    loss_list = []
