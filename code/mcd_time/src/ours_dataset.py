...
class Oursdataset(Dataset):
    def __init__(self, base_path, path_vid):
        ...
        for i in self.hotnum_complete:
            if i["video_id"] in self.vid:
                self.video_time[i["video_id"]] = i["create_time"]
        # 评论发布时间数据, 时间间隔（小时）
        self.comment_times = {}
        for item in self.hotnum_complete:
            if item["video_id"] in self.vid:
                ...
                for com in item['comments']:
                    self.comment_times[item["video_id"]].append(time_span(com["comments_time"], item["create_time"]))
    ...
    def __getitem__(self, idx):
        ...
        # print("真实评论数:", item["comment_lens"])  # 在 __getitem__ 里
